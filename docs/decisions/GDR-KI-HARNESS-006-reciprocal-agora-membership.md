...
An Agora owner may also declare canonical HTTPS GitHub repository `references` for working-set projection. References are owner-selected, mutually exclusive with owner and members, need no reciprocal declaration or `.ki.toml`, and remain explicitly classified as non-members without role or authority. A machine-local host may associate each reference with one selected matching Git checkout; unresolved references yield typed diagnostics and are omitted without weakening reciprocal member results.
