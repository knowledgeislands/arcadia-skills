...
- **`preferred_model_type`**, validated against `['frontier', 'reasoning', 'standard', 'fast']`, is the persistent configuration contract. `ki-subagents`' `FM-2` keeps the Claude `model:` alias (it _is_ the Claude Code subagent spec) but its rationale now traces to the type the role needs, of which the alias is the runtime's resolution.
- **A new optional `["knowledgeislands/ki-agentic-harness:ki-tokenomics".model_tier_bindings]` sub-table** rebinds each type to the concrete model(s) a runtime supports — this is what makes the contract portable rather than a rename. **Keys are strict** (each must be one of the four types; an unknown key is a FAIL — rubric `CFG-5`). **Values are open, comma-separated, ordered preference lists**: `reasoning = "opus, gpt-5.6-sol"`, and each runtime resolves the first model it recognises. A single config is therefore portable across runtimes. Individual model names stay open strings — the checker holds no model ids, which are runtime-specific and volatile.
- **The concrete resolution lives in the public [prompting guides](https://knowledgeislands.info/guidance/prompting/)**, not in the standard or the checker: the Claude column in the per-tier guides, the Codex column in the GPT guide, and the cross-runtime table in the prompting index. Preview-sourced mappings remain flagged for reconfirmation on `ki-tokenomics` REFRESH.
...
- `ki-subagents` — `FM-2`, the per-agent `model:` pin that cites the taxonomy.
- [Prompting guides](https://knowledgeislands.info/guidance/prompting/) — the type → model resolution per runtime.
