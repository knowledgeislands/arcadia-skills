...
- **Run repository operations natively.** The host reads the selected repository's `.ki.toml`, requires all explicit dependencies to be declared, and resolves the resulting operations only from verified installed compatible harnesses. It does not write `package.json`, user-global payloads, hooks, or settings.
...
- A repository remains declarative through `.ki.toml`; a clean clone needs the verified compatible harnesses before it can execute governance operations. It no longer carries a committed checker copy, aggregate runner, or execution manifest.
