...
2. **Learn-block context check.** The `headroom:learn` block, wherever it appears, must not carry entries rooted in another repo. Detection is mechanical and keyed on absolute `knowledgeislands/<repo>` paths _inside the markers_ whose `<repo>` differs from the audited repo — a WARN, with the fix (re-learn or prune) left to CONFORM. It is applied by `ki-housekeeping-claude` over the selected repository's `MEMORY.md` and by `ki-tokenomics-claude` over its project `CLAUDE.md`, consistent with the two-axis split: housekeeping owns Claude-state hygiene, while portable `ki-tokenomics` owns the standing-surface budget.
