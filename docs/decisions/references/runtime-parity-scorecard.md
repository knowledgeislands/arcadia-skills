...
- **◐** — partial: native source support exists, but publication, activation, or runtime evidence is incomplete.
...
| Agents | Native MD+YAML adapter | Native TOML adapter | ◐ | Portable parent and both source adapters; no host publisher. § |
...
§ ADR-KI-HARNESS-AGENTS-002 makes `ki-subagents` the portable semantic parent and gives Claude Code and Codex separate native source adapters. Codex documentation defines standalone TOML under `~/.codex/agents/` and `.codex/agents/`; Claude Code uses Markdown with YAML frontmatter. The current `tools-ki` host advertises Claude subagent capability metadata but implements no publisher for either runtime, so installation, activation, and executed-runtime support remain unavailable.

¶ `plan-stamp`/`plan-sync` ride Claude Code's native Plan Mode, which the matrix marks **None known** for Codex. Parity here likely means a KI-invented reimplementation of the capability, not a port — the way `ki-change-management-roadmap`/`ki-plan` are already pure KI inventions with zero runtime dependency.
...
The develop-in-both milestone is closed. Explicitly **off** that path, still open as their own future items: **agents** (runtime-selective host publication and activation), **hooks** (Phase 3 generator plus the Plan-Mode question), and **evals** on Codex. These are governance and automation depth, not blockers to concurrent development.
