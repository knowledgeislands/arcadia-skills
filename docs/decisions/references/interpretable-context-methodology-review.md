...
| Scoped context | MWP stage contracts name the exact input files and sections needed for one stage. | `ki-delegate` requires a cold-agent-ready brief with bounded scope, locked/escalate decisions, and a verification gate. | **Aligned, with a potential refinement.** The `ki-delegate` owner may consider an explicit source-artifact locator in its worker-brief template; it is not required to implement this review. |
...
The only potential improvement is for the `ki-delegate` owner to decide whether a worker brief should consistently name the durable source artifacts a cold worker must read.
...
- `ki-delegate` — durable, bounded worker briefs and an orchestrator integration gate.
