...
- [ ] Replace the old physical taxonomy decision with the concern-first directory model and explicit `ki-kind` frontmatter contract.
- [ ] Add and mechanically enforce `ki-kind: governance | process` for every KI skill; use it for mode and shape routing.
- [ ] Move the forward-work process skills into `skills/change-management/` and move `ki-roadmap` and `ki-housekeeping` there with their governance kind unchanged.
- [ ] Introduce `ki-delegation` under `skills/governance/` with sources, a delegation-packet standard, and a native rubric that audits declared delegation sections and safely repairs only mechanical packet omissions.
- [ ] Reduce `ki-delegate` to the process that applies `ki-delegation` to an approved plan or delivery run, retaining no duplicate standard.
- [ ] Update all source references, discovery/projection paths, diagrams, configuration, generated rubrics, and tests.
- [ ] Verify focused migrations, the full test and type gates, and affected native audits.
