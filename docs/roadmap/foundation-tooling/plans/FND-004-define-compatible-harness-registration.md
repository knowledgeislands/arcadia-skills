...
8. Retire the harness's repository-local `.ki/` executor only after `ki repo educate`, `ki repo audit`, and `ki repo conform` can replace its aggregate, CI, and pre-commit roles for this source harness.
   ...
   - [ ] Execute the explicit, fail-closed source-harness deletion: remove each migrated skill's `scripts/govern.ts`, `scripts/educate.ts`, vendored execution payload, and wrapper-only test only after its native role has passed; then remove the repository `.ki/` aggregate, bootstrap payload, manifest, aliases, and superseded bootstrap/guide fixtures. Retain or relocate `.ki/self` deliberately rather than deleting it as collateral.
