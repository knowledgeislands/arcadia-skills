...
Twenty-two in-scope structured catalogues still use transitional catalogue or context adapters, and those are the remaining skill reviews after accepting `ki-skills`, `ki-authoring`, `ki-handoffs`, and the guidance-only `ki-bootstrap`. The deferred `ki-specifications` catalogue remains outside this count.
