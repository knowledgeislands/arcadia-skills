...
8. Retire the harness's repository-local `.ki/` executor only after `ki repo audit` and `ki repo conform` can replace its aggregate, CI, and pre-commit roles for this source harness.
   ...
   - [ ] Convert every wrapper-bearing governance skill to the native definition contract, replacing direct-writing callbacks only after the tools-owned transaction preserves its existing fail-closed safety guarantees.
     ...
     - [x] Add explicit `ki user audit` / `ki user conform` host scope for `ki-housekeeping` and `ki-tokenomics`: native user scopes declare bounded home-relative paths, inherit containment and identity checks, and reject subprocess repairs rather than smuggling user work through `ki repo`.
     ...
     - [ ] Port the remaining repository-scoped wrappers: `ki-dotfiles-chezmoi`, `ki-homebrew-tap`, `ki-mcp`, `ki-plugins`, and `ki-tools`.
     ...
     - [ ] Port the remaining specialised wrappers: `ki-kb-activities`, `ki-kb-live-artifacts`, `ki-kb-streams`, `ki-website`, and `ki-website-cloudflare`; port `ki-specifications` last, after the active implementation migration is stable.
     - [ ] Retire `scripts/rubric/publish.ts` as each native definition lands: regenerate `references/rubric.md` through `ki skill rubric <skill> --write`, remove its publisher and publisher-only test, and replace every authoring instruction with the `ki` command. Complete the cross-skill authoring guidance once the final publisher is gone.
       - [x] Retire the publishers for the first 16 native skills: `ki-authoring`, `ki-binding`, `ki-binding-chezmoi`, `ki-bootstrap`, `ki-decision-records`, `ki-engineering`, `ki-feature-definitions`, `ki-handoffs`, `ki-harness`, `ki-housekeeping`, `ki-kb`, `ki-repo`, `ki-repo-roadmap`, `ki-skills`, `ki-subagents`, and `ki-tokenomics`.
     ...
     - [ ] After the wrapper, publisher, vendored-engine, and source `.ki/` cleanup, perform a high-reasoning manual review of every native `scripts/rubric/index.ts`: remove transitional `RubricItem` casts and repeated mechanical/judgment mappers, retaining only native contract data, read-only evidence contexts, and skill-specific repair policy. Do not move this legacy adapter into `tools-ki`, which must consume native definitions only.
