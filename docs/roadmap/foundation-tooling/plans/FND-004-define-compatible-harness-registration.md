...
4. Apply the target taxonomy as one clean cut after the active per-skill edits land. Adopt `ki-roadmap` as the roadmap-governance capability name; keep `ki-repo-review`; consolidate the governance skills; and move the remaining skills into `agentic-systems`, `environment`, `knowledge-bases`, `tooling`, and `websites` as shown above. Update frontmatter dependencies, `.ki-config.toml`, generated graphs, documentation, fixtures, and every source or link reference in the same change. Do not move or edit `ki-specifications` during this plan.
