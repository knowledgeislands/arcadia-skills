...
This item changes the current operating contract, not historical evidence. New and current documentation, configuration, skill trees, checkers, publication surfaces, and corpus roots use only `ki-specs` and `docs/specs/` when this work is complete. GDR-KI-HARNESS-004 and ADR-KI-HARNESS-SKILLS-008 are current living Decision Records and change in place with the contract. Roadmap items and trade records retain their original wording as history; they are not compatibility surfaces and must not be rewritten merely to erase a search result.
...
The existing `ki-specifications` capability governs the structure of the separate KI Specifications repository. It is outside this item and remains unchanged.
...
The Harness migration is in progress: its canonical skill, declaration, and corpus now use `ki-specs` and `docs/specs/`. tools-ki and vallearmonia-website remain on the former contract until their ordered lanes run. No compatibility path is being introduced.
...
- [ ] Rename the canonical Harness skill from `ki-feature-definitions` to `ki-specs`, including its declared name, checker capability, standards, rubric, evaluator fixtures, and active references. Do not leave a compatibility alias.
...
- [ ] Update the published skill copy, documentation site, and tools-ki host so they expose only `ki-specs` and the `docs/specs/` contract.
...
The tools-ki host rejects `ki-feature-definitions` and recognises only `ki-specs`. An active-surface search finds no `docs/features/`, `docs/spec/`, or `ki-feature-definitions` footprint outside intentionally historical roadmap and trade records. Repository test and TypeScript gates pass where source changes occur.
