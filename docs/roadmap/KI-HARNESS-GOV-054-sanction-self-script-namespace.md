...
- **Authority:** Create and bind the receiver-local outcome authorisation, shape and deliver the named record, apply the locked renames and live-reference updates, alphabetise the resulting `package.json` script keys without changing command bodies, verify, and commit; do not touch or stage `apps/site/src/styles.css`, change `ki:site:*`, contact external systems, close, prune, or push.
