...
- **The user guide** ([`guides/user/`](guides/user)) is a one-sitting account of the harness for someone using it. It explains the harness on its own terms and names decisions and skills by their ID or `name:` rather than linking into `decisions/` or `skills/`. A reader finishes it without ever leaving it.
