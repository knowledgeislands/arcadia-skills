...
`ki-housekeeping-claude` recognises this boundary: it governs accumulated Claude machine state, while `ki-self` governs the repository-local concerns that sit beside it. The [single-page skills illustration](../../diagrams/skills-map.svg) places the shared foundations on the left and specialised capabilities on the right; colour and each node’s second line identify its family. `ki-self` remains visibly distinct as repository-local governance, with a promotion edge back to a named shared skill.
