...
Optional user- and system-level tools that affect a person's machine rather than a governed repository. Skills, MCP servers, and day-to-day sessions can benefit from them, but none is required merely to read or audit the harness. [Install and get started](getting-started.md) explains `ki bootstrap`, explicit skill activation, and when optional user-environment setup is appropriate.
