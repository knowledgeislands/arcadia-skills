...
- **Mechanical** — deterministically checkable. The bundled [`scripts/govern.ts`](scripts/govern.ts) `audit` command runs these: file exists, frontmatter parses, `name` matches the directory and the charset rules, length caps, link resolution, no wikilinks. **Always run the linter first** — do not eyeball what a script checks better.
...
**Auditing a whole repo? Keep mechanics central; isolate only judgment** ([ADR-KI-HARNESS-AGENTS-001](../../../docs/decisions/ADR-KI-HARNESS-AGENTS-001-subagent-isolation-for-multi-skill-invocation.md)): run the linter's whole-repo pass (COLL-1) and the description-reciprocity review (COLL-2) in the main context. For a large review, use `ki-delegate` to fan out only independent [J] review after capturing the mechanical output; each reviewer receives one bounded concern and returns findings for the orchestrator to synthesise. Rank findings by dependency order (foundations first) per [ADR-KI-HARNESS-SKILLS-003](../../../docs/decisions/ADR-KI-HARNESS-SKILLS-003-dependency-order-for-multi-skill-composition.md) — that order is synthesis priority, not execution order. Do not maintain a runtime-specific workflow to re-run the checker fleet.

1. **Run the linter.** For a human-readable report of one skill, run `bun scripts/govern.ts audit <path-to-skill> --reporter=terminal` from this skill's directory. Omit `--reporter=terminal` only when consuming the complete [canonical checker response](references/checker-response.md) as JSONL. To audit the whole harness through its aggregate reporter, run `bun run ki:skills:audit` at the repository root. The checker uses `FAIL`, `WARN`, `INFO`, `NOT_APPLICABLE`, and `PASS`, and exits non-zero on any `FAIL` finding. Judgment aspects are not emitted as synthetic findings; the response summary counts them as unevaluated. Capture the response verbatim — do not re-derive what it found. Use the repository as the target when the cross-skill collision pass (COLL-1) must compare siblings.
