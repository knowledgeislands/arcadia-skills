...
1. **Measure the footprint.** The current `ki` host does not expose the former optional per-skill footprint projection, so SIZE-5 is neutral and not applicable during repository audit. Use `ki-tokenomics` for the environment-level `skills_surface`; restore a per-skill projection only through an explicit host capability rather than a private runner.
