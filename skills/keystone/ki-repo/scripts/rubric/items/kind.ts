...
const KIND_1: RubricItem<KindRubricContext> = {
  ...
  sources: [SOURCE],
  mechanical: { level: 'FAIL', audit: { phase: 'INSPECT', run: (context) => auditEvidence(context.kind1, 'FAIL') } }
}
...
const KIND_2: RubricItem<KindRubricContext> = {
  ...
  sources: [SOURCE],
  mechanical: { level: 'FAIL', audit: { phase: 'INSPECT', run: (context) => auditEvidence(context.kind2, 'FAIL') } }
}
