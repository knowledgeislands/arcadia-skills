...
const KIND_1: RubricItem<KindRubricContext> = {
  ...
  description: 'ki-repo owns the optional KB discriminator and validates its closed named-store vocabulary without accepting legacy locations.',
  sources: [SOURCE],
  mechanical: {
    level: 'FAIL',
    remediation: { class: 'diagnostic', guidance: 'Declare a supported repository kind and compatible store roles, then rerun the audit.' },
...
const KIND_2: RubricItem<KindRubricContext> = {
  ...
  description: 'A KB kind declares the KB structure and Streams planning model; a non-KB does not declare the KB structure.',
  sources: [SOURCE],
  mechanical: {
    level: 'FAIL',
    remediation: { class: 'diagnostic', guidance: 'Align the repository kind with its declared structure and planning model, then rerun the audit.' },
