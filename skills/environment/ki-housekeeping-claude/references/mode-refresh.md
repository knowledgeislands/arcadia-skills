...
3. **Diff and update** [standards-auto-memory.md](standards-auto-memory.md), `SKILL.md`, and the canonical TypeScript items under `scripts/rubric/items/` where the live behavior has moved; regenerate [rubric.md](rubric.md) with `ki dev skill rubric ki-housekeeping-claude --write`.
