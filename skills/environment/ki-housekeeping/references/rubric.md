...
- **SELF-1 [M] — Repository-local ki-self source** — The repository owns one regular `ki-self` source at `.agents/skills/ki-self/SKILL.md`. Missing is a WARN; a symlink or non-regular source is a FAIL. (standards.md)
- **SELF-2 [M] — ki-self source name** — The source declares `name: ki-self`. A mismatch is a FAIL. (standards.md)
- **SELF-3 [M] — Claude runtime projection** — When `claude-code` is declared, `.claude/skills/ki-self` is a relative link to the canonical source. A missing or divergent projection is a FAIL. (standards.md)
