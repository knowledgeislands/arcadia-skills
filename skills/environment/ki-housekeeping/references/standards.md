...
- For **memory**, the skill governs the area in full — the format is a file convention the skill fully specifies ([memory-format.md](memory-format.md)), declares with `ki repo educate --skill ki-housekeeping`, and checks with `ki repo audit --skill ki-housekeeping`. No server tool is needed to read the bounded Claude project-memory store.
