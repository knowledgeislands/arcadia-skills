...
| **Auto-memory**         | The per-project `memory/*.md` + `MEMORY.md` Headroom writes             | **Native KI rubric** + memory-format.md |
...
- For **memory**, the skill governs the area in full — the format is a file convention the skill fully specifies ([memory-format.md](memory-format.md)) and checks with `ki user audit --skill ki-housekeeping`. No server tool is needed to read the bounded Claude project-memory store.
