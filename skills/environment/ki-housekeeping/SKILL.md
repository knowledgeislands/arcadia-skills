...
| CONFORM | Run `ki repo conform --skill ki-housekeeping`; it performs only safe transactional memory-file repairs. Use the server's access-gated cleanup tools for other areas (destructive tools require the server's access level). It does not invent a repository's local concerns or `ki-self` body; use that repository's local EDUCATE/CONFORM process. Re-AUDIT until clean. Same procedure file as AUDIT. |
