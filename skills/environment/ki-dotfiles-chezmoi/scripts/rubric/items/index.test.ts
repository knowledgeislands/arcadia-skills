...
const fixture = (): string => {
  const repository = mkdtempSync(join(tmpdir(), 'ki-dotfiles-chezmoi-'))
...
test('the catalogue preserves every chezmoi criterion in family order', () => {
  ...
  expect(catalogue.families.map((family) => family.code)).toEqual(['CHEZMOI', 'BIN', 'GIT', 'PATTERN', 'CONFIG', 'LAYER', 'ETIQ', 'SYNC', 'RUBRIC'])
