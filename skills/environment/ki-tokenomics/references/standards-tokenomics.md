...
Only `[ki-tokenomics]` is read, validate-down. Recognised scalar keys are `headroom` (`required`, `recommended`, or `off`), positive integer `context_window_tokens`, and `preferred_model_type` (`frontier`, `reasoning`, `standard`, or `fast`). `budgets` may contain only positive numeric values for the default budget categories. `model_tier_bindings` may contain only portable model-type keys with non-empty strings. Unknown keys WARN; malformed recognised values FAIL.
