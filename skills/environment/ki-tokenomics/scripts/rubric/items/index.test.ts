...
describe('ki-tokenomics rubric catalogue', () => {
  ...
  test('keeps policy findings diagnostic and publication automatic', () => {
    const items = definition.families.flatMap((family) => family.items as readonly unknown[]) as readonly { code: string; mechanical?: { remediation: { class: string } } }[]
    expect(items.filter((item) => item.mechanical?.remediation.class === 'diagnostic').map((item) => item.code)).toEqual(['CFG-1', 'POL-1', 'POL-2', 'POL-3'])
