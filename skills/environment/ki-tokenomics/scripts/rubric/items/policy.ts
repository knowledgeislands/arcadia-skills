...
const POL_1: RubricItem<TokenomicsConfigContext> = {
  ...
  sources: [SOURCE],
  mechanical: { level: 'WARN', remediation: { class: 'diagnostic', guidance: 'Revise the selected budget guide-rail or record its intended overage, then rerun the audit.' }, audit: { phase: 'INSPECT', run: (context) => context.budgetPolicy } }
}
const POL_2: RubricItem<TokenomicsConfigContext> = {
  ...
  sources: [SOURCE],
  mechanical: { level: 'WARN', remediation: { class: 'diagnostic', guidance: 'Use a declared portable model purpose rather than a provider-specific model choice, then rerun the audit.' }, audit: { phase: 'INSPECT', run: (context) => context.modelPurpose } }
}
const POL_3: RubricItem<TokenomicsConfigContext> = {
  ...
  sources: [SOURCE],
  mechanical: { level: 'WARN', remediation: { class: 'diagnostic', guidance: 'Route the standing-surface finding to its owning artifact or runtime adapter, then rerun the audit.' }, audit: { phase: 'INSPECT', run: (context) => context.routing } }
}
