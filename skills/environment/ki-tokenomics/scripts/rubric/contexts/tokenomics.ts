...
export const createTokenomicsSession = ({
    ...
    config: {
      validates: invalid.length
        ? one(
            outcome(
              'VIOLATION',
              `Malformed ["knowledgeislands/ki-agentic-harness:ki-tokenomics"] value(s): ${invalid.join(', ')}`,
              'FAIL'
            )
          )
        : unknown.length
          ? one(
              outcome(
                'VIOLATION',
                `Unknown ["knowledgeislands/ki-agentic-harness:ki-tokenomics"] key(s): ${unknown.join(', ')}`,
                'WARN'
              )
            )
          : one(
              outcome(
                'PASS',
                'Selected repository ["knowledgeislands/ki-agentic-harness:ki-tokenomics"] configuration validates down.'
              )
            ),
  ...
  return {
    subjects: [
      {
        families: ['CFG', 'POL'],
        subject: '["knowledgeislands/ki-agentic-harness:ki-tokenomics"]',
        context: () => context
      },
