...
const CODEXBIND_1: RubricItem<CodexBindingContext> = {
  ...
  mechanical: {
    ...
    remediation: {
      class: 'diagnostic',
      guidance:
        'Review the canonical source and run the native Codex renderer after confirming the intended client targets; do not overwrite unrelated application configuration.'
    },
    audit: {
      phase: 'INSPECT',
        ...
        servers === null
          ...
          : [...expected].every((name) => servers.has(name))
            ? [
                {
                  status: 'PASS',
                  message: `Codex TOML contains all ${expected.size} targeted server(s).`,
                  subject: configPath
                }
              ]
            : [
...
const CODEXBIND_J1: RubricItem<CodexBindingContext> = {
  ...
  judgment: {
    ...
    prompt:
      'Does the native Codex writer still preserve non-KI application configuration better than whole-file ownership would?',
    outcomes: ['conforming', 'merge boundary revision required', 'ownership decision required'],
    guidance:
      'Keep native merge ownership where it preserves unrelated configuration; otherwise record the owning runtime decision before changing the write boundary.'
  }
}
