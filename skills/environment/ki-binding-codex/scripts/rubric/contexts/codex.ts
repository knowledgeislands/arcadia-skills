...
export type CodexBindingContext = { configPath: string; servers: ReadonlySet<string> | null; expected: ReadonlySet<string> }
...
export const createCodexBindingSession = ({ repository, userHome }: RubricContextOptions): RubricSession<CodexBindingContext> => {
  ...
  const context = { configPath, servers: inspect(configPath), expected: expected(source) }
  return { subjects: [{ families: ['CODEXBIND'], context: () => context, subject: resolve(repository) }], proposal: () => ({ writes: [] }) }
}
