...
This is a **standard, base-agnostic Process skill**. It hard-codes no single repo; it applies to any repo carrying a `[ki-website-cloudflare]` table in its `.ki-config.toml`. How it sits beside the other skills, and where it must not overlap them, is documented once in the ki-agentic-harness `README.md`.
...
The full, quotable standard is [the Cloudflare hosting standard](references/standards.md); the line-by-line items are in [the audit rubric](references/rubric.md); the tracked provenance is [the source list](references/sources.md). `ki repo audit --skill ki-website-cloudflare` runs the mechanical checks. Read those for detail; this file is the operating procedure.

## The model at a glance

```jsonc
// <site root>/wrangler.jsonc — the site Worker. (Site root is repo root if flat, site/ if the
// repo also holds other deployables — see ki-website.)
{
  "name": "<site-name>",
...
2. **`assets.directory` is the seam.** It points at the `dist/` that `ki-website` emits (`./dist` flat, `../dist` from a `site/` subfolder). The build runs before deploy; `dist/` is gitignored.
...
ki:engineering:audit <repo>                          →  common toolchain (Bun, lint/deps families, tsconfig/biome)
  then audit.ts <repo>                   →  site-build delta (ki-website) → produces dist/
  then audit.ts <repo>         →  serving the dist/ (THIS skill)
```

A repo is "clean" only when **every applicable** skill's audit passes. The `[ki-website-cloudflare]` table in `.ki-config.toml` is this layer's selector.

## The `dist/` contract (the seam from the build)

This skill consumes exactly what `ki-website` emits: a portable `dist/` of static files with relative internal links. The hosting job is to point `assets.directory` at it and deploy. **Building `dist/` is the 11ty skill; serving it is this one.** The two meet only at the `dist/` path — neither needs the other's internals.
...
2. **Run the mechanical audit.** `ki repo audit --repo <repo> --skill ki-website-cloudflare` finds the **site** `wrangler.jsonc` (the one with an `assets` block), then reports: present at the site root, `assets.directory` set and pointing at `dist/`, `name` + `compatibility_date`, `observability.enabled`, a `wrangler deploy` script, **not** `wrangler pages deploy`, and `dist/` + `.wrangler/` gitignored. It also reads this skill's `[ki-website-cloudflare]` table: the opt-in marker, plus the one declarable key `site-root` (validate-down warns on any other key, and on a `site-root` that holds no `wrangler.jsonc`). It notes (does not flag) any companion Worker. Cloudflare account, domain, and Wrangler operations remain explicit manual work.
...
2. Fix the gaps in place — use the canonical shape from [the standard](references/standards.md): the `wrangler.jsonc` shape (`assets.directory`, `routes`, `observability`) and the `site:{deploy,preview,clean}` scripts. Adapt name, domains, and the `dist/` relative path to the layout. Add the `[ki-website-cloudflare]` table if missing, then set `site-root`. If a `wrangler pages deploy` is found, migrate it to Workers + Static Assets.
3. Re-run the native audit; `bunx wrangler deploy --dry-run` from the site root remains an explicit validation step and should read the assets directory cleanly.
...
2. **Re-fetch each** (WebFetch / WebSearch) and diff against the standard + rubric + native rubric definition: new/changed `assets` keys (`html_handling`, `not_found_handling`, `run_worker_first`), `compatibility_date` guidance, and whether Pages' deprecation status for static sites has moved.
...
- **Building the `dist/`** — Eleventy, Nunjucks, Tailwind, the portable-URL transform, the `src/` layout → `ki-website`. The `dist/` is the seam between the two.
