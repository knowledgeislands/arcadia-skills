...
1. **Run the mechanical checker** — `ki repo audit --skill ki-kb-activities`. It checks the activity index, declared `status` and `realization`, realization-specific fields, and the optional `harness` path in the skill's `.ki-config.toml` table. Unknown realizations and external scheduled-task registration are non-blocking `INFO`; apply the judgment criteria by reading. Exit code is non-zero on any FAIL.
...
2. Run `ki repo conform --skill ki-kb-activities` to add missing activity-index entries. The native host creates `Activities.md` only when activity notes exist and no index is present.
