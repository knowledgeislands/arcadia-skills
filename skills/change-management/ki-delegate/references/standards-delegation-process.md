...
_On-demand procedure for `ki-delegate`. The kind, scope, and leg summary live in [`SKILL.md`](../SKILL.md) and are already loaded; this file owns the operational five-leg procedure._

`ki-delegation` owns the delegation-packet artifact, its required fields, sources, and audit/conform boundary.

Before running this process, make the selected plan conform to that standard.

## Contents

- [Delegation-ready input](#delegation-ready-input)
- [Decide dispatch](#1-decide-dispatch)
- [Classify](#2-classify)
- [Assign](#3-assign)
- [Sequence](#4-sequence)
- [Gate](#5-gate)

Throughout, **`(CC)`** marks a mechanism specific to the Claude Code runtime. The method itself — prepare a delegation-ready brief, decide dispatch, then classify, assign, sequence, and gate — is runtime-neutral; only the spawning mechanics are `(CC)`. A different runtime substitutes its own spawn primitive at those points and the method is unchanged. This mirrors the SPEC/CC tagging discipline `ki-skills` applies to skill frontmatter.

## Delegation-ready input

Reason once at the planning or orchestration layer, then capture that reasoning in the `ki-delegation` packet so workers do not reconstruct hidden context.

Apply the cold-agent readiness test: could an agent with no conversation history or planner context execute its first step from the packet, while knowing what is fixed and when to stop for a decision?

If not, refine the packet, split the unit, or retain the work with the orchestrator.

Cross-repository transfer is outside this process; the receiving repository owns adoption, priority, and planning through its roadmap lifecycle.

## 1. Decide dispatch

Make the context decision before selecting an agent or model. It has exactly three outcomes, using the existing brief, cold-agent readiness, minimum-viable-model, and escalation vocabulary:

- **Retain origin only when essential reasoning cannot be made durable.** This is an exception for active synthesis or a decision whose necessary evidence, alternatives, and constraints cannot yet be faithfully recorded. Name the missing durable reasoning and the decision checkpoint; do not use warm context as an excuse to omit the brief or the gate. Preserve the best available brief before continuing so the work can recover after interruption.
- **Use a fresh worker when the cold-agent readiness test passes.** The brief, not a context fork or a compacted summary, is the evidence that the work is transferable. A fresh worker receives the same locked decisions, escalation boundaries, definition of done, file scope, model, and verification gate as any other delegated unit.
- **Use stronger reasoning only for decision risk.** A stronger model or specialist is justified where an incorrect framing is costly to unwind. It is not a substitute for a durable brief, and retained origin context does not itself justify a stronger model.

Before a fresh dispatch or native context compaction, preserve and verify the brief against the current repository state. Drain and gate every delegated worker before compaction. A forked or compacted conversation is never evidence by itself: re-ground the brief, plan, and repository facts after the boundary.

### Worked decisions

**Retain origin — unresolved standard synthesis.** The orchestrator is reconciling two source standards into a new policy and cannot yet state the trade-off without losing material alternatives. Retain that judgment task with the originating agent, record the sources, alternatives, locked boundary, and next decision checkpoint in its brief, then dispatch any resulting mechanical edits only after the decision becomes durable. The task does not bypass review or verification.

**Fresh worker — bounded source research.** The brief asks a worker to locate the documented configuration field for a named external tool, cite primary documentation, and report only the field, source, and uncertainty. Its scope, definition of done, escalation rule, and verification are complete without the prior conversation, so it passes the cold-agent readiness test and may use the cheapest model sufficient for reliable source discovery.
...
- Research tasks **gate** — schedule them early (see §4), because judgment and mechanical work downstream of an unknown cannot be trusted until it resolves.
...
- **Mechanical** → a general worker agent given the exact spec, on the **cheapest sufficient** model. Precise spec plus low ambiguity makes the work safe to delegate, and the orchestrator reviews the diff (§5).
...
4. **Keep the orchestrator in the loop.** The orchestrator dispatches a round, gates its results (§5), then dispatches the next — it does not fire every round blind. Background spawns `(CC: run_in_background)` are for concurrency within a round, not for skipping the gate between rounds.
...
- **Re-check the state facts a report asserts, not just its verdict.** A worker's conclusion is reviewed above; the incidental claims it makes in passing are not. Statements like "the other repositories already declare this", "that file is untracked", or "this predates the change" are read as established, then used to size or skip the next task — while the worker may have inferred them, seen them before its own edits, or observed a tree another agent has since moved. Re-derive any such claim that will shape a decision (`git log`, a fresh read, the actual config), and treat a report's scope claims as bounded by what it demonstrably inspected. A worker that reasons well can still assert a wrong fact about a repository it only glanced at.
