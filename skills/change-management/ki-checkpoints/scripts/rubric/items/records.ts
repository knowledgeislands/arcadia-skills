...
const RECORD_1: RubricItem<RecordContext> = {
  ...
  sources: [SOURCE],
  mechanical: { level: 'FAIL', heuristic: true, remediation: { class: 'guarded', guidance: 'Correct record identity only through an explicit user-selected thread update.' }, audit: { phase: 'INSPECT', run: (context) => context.identity } },
  judgment: { scope: 'Every checkpoint filename, thread field, and H1.', prompt: 'Is each thread name a stable human-selected lookup key rather than a vendor or runtime identifier?', outcomes: ['conforming', 'explicit rename required', 'escalate to user'], guidance: 'Keep the user-selected thread as the lookup key; do not derive or replace it from runtime metadata.' }
}
...
const RECORD_2: RubricItem<RecordContext> = {
  ...
  sources: [SOURCE],
  mechanical: { level: 'FAIL', remediation: { class: 'guarded', guidance: 'Correct authored record schema only through an explicit checkpoint update.' }, audit: { phase: 'INSPECT', run: (context) => context.schema } },
  judgment: { scope: 'The authored frontmatter, headings, and reconstruction sections of every record.', prompt: 'Can the schema correction be made without inventing or discarding user-owned reconstruction state?', outcomes: ['conforming', 'explicit update required', 'escalate to user'], guidance: 'Ask the user to update uncertain authored content; the checker must not infer missing checkpoint state.' }
}
