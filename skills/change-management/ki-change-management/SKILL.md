...
Run `ki repo conform --skill ki-change-management --repo <repo> --dry-run`. CONFORM makes no selection, creates no tracker, and changes no work record. It never infers an adapter from repository shape; set the declaration deliberately, then audit the selected adapter.

### Mode EDUCATE

Run `ki repo educate --skill ki-change-management --repo <repo>` to render the adapter contract. Choose the Project or KB default only after confirming its primary structure; choose GitHub Issues or Linear only after confirming remote authority and the owning adapter configuration.
