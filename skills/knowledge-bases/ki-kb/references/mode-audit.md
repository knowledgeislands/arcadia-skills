...
3. Confirm that the host ran the declared focused prerequisites for Activities, Live Artifacts, and Streams before `ki-kb`. Run separately declared coverage such as `ki-authoring` for Markdown conventions. A base is clean only when every applicable governing skill is clean.
