...
- **Reference-implementation gap.** The skill has no live, conformant reference base. Either conform ki-arcadia-principal's Activities notes (add `realization`, switch status to the enum, add a `[skills.ki-kb-activities]` table) so the citation is honest, or reclassify the citation as a pre-adoption sample. Decide direction before next REFRESH.
