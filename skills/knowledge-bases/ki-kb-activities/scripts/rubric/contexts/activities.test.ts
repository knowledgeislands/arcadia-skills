...
const activityBase = (): string => {
  const repository = temporaryDirectory('ki-kb-activities-')
...
const activityContext = (session: ReturnType<typeof createActivitiesSession>) => {
  ...
  if (!subject) throw new Error('ki-kb-activities session did not expose its repository subject')
...
test('a configured collection path produces one bounded index proposal', () => {
  const repository = temporaryDirectory('ki-kb-activities-alias-')
...
test('an index symlink is neither followed nor proposed for replacement', () => {
  const repository = activityBase()
  const outside = join(temporaryDirectory('ki-kb-activities-outside-'), 'outside.md')
...
test('a configured collection cannot traverse an intermediate symlink', () => {
  const repository = temporaryDirectory('ki-kb-activities-root-')
  const outside = temporaryDirectory('ki-kb-activities-linked-')
