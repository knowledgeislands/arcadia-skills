...
Almost everything is fixed above. Only these come from the host base — take declarative overrides from the base's `.ki-config.toml` `[skills.ki-kb-streams]` table (the shared-file contract is owned by `ki-repo`; validate your own table, warn on an unrecognised key, never read another skill's), otherwise from the auto-loaded `CLAUDE.md`.
...
1. Resolve the bindings: read the base's `.ki-config.toml` `[skills.ki-kb-streams]` table and `CLAUDE.md`. **This skill is the canonical definition** of the Enactment Process; load the base's bound **process note** if it has one, for its local specifics (it points back here).
