...
const fixture = (): { readonly repository: string; readonly config: string; readonly original: string } => {
  const repository = temporaryDirectory('ki-homebrew-tap-')
...
const rootContext = (session: ReturnType<typeof createHomebrewTapSession>) => {
  ...
  if (!subject) throw new Error('ki-homebrew-tap session did not expose its repository subject')
...
test('a symlinked config is reported but never proposed for replacement', () => {
  const repository = temporaryDirectory('ki-homebrew-tap-root-')
...
test('an unrelated repository is not applicable', () => {
  const repository = temporaryDirectory('ki-homebrew-tap-unrelated-')
  const session = createHomebrewTapSession(options(repository, 'audit'))
  const { context } = rootContext(session)
