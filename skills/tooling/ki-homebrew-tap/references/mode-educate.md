...
4. **Wire governance.** Add `["knowledgeislands/ki-agentic-harness:ki-repo"]` + `["knowledgeislands/ki-agentic-harness:ki-homebrew-tap"]` to `.ki-config.toml`—after `ki-repo` establishes the shared file, `ki repo conform --skill ki-homebrew-tap --repo <tap-path>` adds the keyless `["knowledgeislands/ki-agentic-harness:ki-homebrew-tap"]` marker safely. Run `ki-repo`'s EDUCATE for the repo-level files and GitHub settings. Do **not** add `["knowledgeislands/ki-agentic-harness:ki-engineering"]`—a tap has no TypeScript toolchain (the `ki-plugins` precedent).
5. **Verify.** Run `ki repo audit --skill ki-homebrew-tap --repo <path>` to 0 FAIL, then run `brew style` and `brew audit --strict` explicitly on each formula. Confirm `brew install <owner>/<x>/<tool>` works before publishing.
