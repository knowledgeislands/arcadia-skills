...
2. **Apply the safe hosted repair.** After `ki-repo` has established a regular `.ki-config.toml`, run `ki repo conform --skill ki-homebrew-tap --repo <tap-path>`. The only automatic change is adding the keyless `["knowledgeislands/ki-agentic-harness:ki-homebrew-tap"]` marker to that existing valid file; missing, malformed, symlinked, or otherwise unsafe configuration remains report-only.
