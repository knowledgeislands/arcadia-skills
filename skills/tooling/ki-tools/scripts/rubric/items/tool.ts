...
const TOOL_BIN = mechanical('TOOL-BIN', 'Tool executable', '`bin/` exists and holds at least one physical file.', 'FAIL', (context) => {
  if (context.rootState === 'absent') return one({ status: 'VIOLATION', message: 'Audit target does not exist.', subject: context.repository })
  if (context.rootState === 'unsafe') return one({ status: 'VIOLATION', message: 'Audit target is not a physical directory.', subject: context.repository })
  const skipped = notApplicable(context)
  if (skipped) return skipped
  if (context.binState === 'unsafe') return one({ status: 'VIOLATION', message: 'bin/ is not a physical readable directory.', subject: 'bin/' })
  ...
  mechanical: {
    ...
    audit: {
      ...
      run: (context) => {
        const skipped = notApplicable(context)
        if (skipped) return skipped
        if (context.install === 'unsafe') return one({ status: 'VIOLATION', message: 'install.sh is not a physical regular file.', subject: 'install.sh' })
        if (context.install === 'missing') return one({ status: 'VIOLATION', message: 'No install.sh exists at the repository root.', subject: 'install.sh' })
...
const TOOL_VERSION = mechanical('TOOL-VERSION', 'Version flag', 'The primary executable successfully runs with `--version`.', 'WARN', (context) => {
  const skipped = notApplicable(context)
  if (skipped) return skipped
...
const TOOL_CHANGELOG = mechanical('TOOL-CHANGELOG', 'Changelog presence', '`CHANGELOG.md` is a physical regular file.', 'WARN', (context) => {
  const skipped = notApplicable(context)
  if (skipped) return skipped
  if (context.changelog === 'unsafe') return one({ status: 'VIOLATION', message: 'CHANGELOG.md is not a physical regular file.', subject: 'CHANGELOG.md' })
...
const TOOL_TESTS = mechanical('TOOL-TESTS', 'Test directory', 'A physical `tests/` or `src/tests/` directory is present.', 'WARN', (context) => {
  const skipped = notApplicable(context)
  if (skipped) return skipped
  if (context.tests === 'unsafe') return one({ status: 'VIOLATION', message: 'tests/ or src/tests/ is not a physical readable directory.', subject: 'tests/' })
...
const TOOL_ENGINEERING = judgment('TOOL-ENGINEERING', 'Engineering declaration', 'A package.json-bearing repository declares ki-engineering.')
const TOOL_LANGUAGE = judgment('TOOL-LANGUAGE', 'Other-language toolchain', 'A non-shell, non-JavaScript tool wires its own lint and test toolchain into CI.')
const TOOL_RELEASE_CHECK = judgment('TOOL-RELEASE-CHECK', 'Release alignment', 'Version markers, tags, releases, and changelog entries agree.')
