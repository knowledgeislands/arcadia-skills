...
5. **Propose a diff** to the standard and native rubric definition (where a check became mechanical). Every new **[M]** item must land as a native checker check. Confirm before writing, then republish [the rubric](rubric.md) with `ki dev skill rubric ki-tools --write`.
