...
- **rumdl:** v0.2.54 released 2026-08-11. Direct `rumdl 0.2.54 check --fix` reproductions now preserve the MD005 nested-list and MD075 table-followed-prose fixtures, so the owned template re-enables both rules. The MD056 wikilink-alias table fixture still truncates after autofix, so it remains disabled. `MD033`, `MD036`, and `MD057` remain separate house/content decisions. The global-settings and CLI references are the authority for config discovery and `check --fix`, rather than the rules page alone.
...
- **Open watch-items:**
  - `MD056` **destroys content** and remains disabled. At 0.2.54, a wikilink alias in a table cell — `[[Target|Label]]` — is still counted as a cell separator and `check --fix` truncates the row to `[[Target |`. Re-test this exact two-column fixture on each upgrade; re-enable only when both cells survive.
