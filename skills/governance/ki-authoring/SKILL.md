...
3. **Report** by location → criterion → fix; lead with FAIL findings, then judgment findings. Submitted records at `+/_TRADES/<owner>/<repo>/TRD-*.md` and `-/_TRADES/<owner>/<repo>/TRD-*.md` are evidence, not authored Markdown: never format, lint-fix, or normalize their frontmatter. The sibling `_PREPARATIONS` tree and the two README files stay fully in scope.
