...
_On-demand procedure for decision-records' REFRESH mode. The cadence and source list are declared in [`sources.md`](sources.md). The full format standard lives in [the Decision Records standard](standards-decision-records.md)._
...
2. **Re-fetch each** (WebFetch; fall back to WebSearch if blocked) and diff against [the Decision Records standard](standards-decision-records.md) and [the generated rubric](rubric.md).
