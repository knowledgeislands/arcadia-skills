...
The repo type is declared in `.ki-config.toml` under `[skills.ki-decision-records]` (or inferred from `[skills.ki-kb]` presence). The checker auto-detects the decisions directory (`docs/decisions/` then `Admin/Governance/Decisions/`) and picks the matching index file by mode; pass an explicit path to override.
