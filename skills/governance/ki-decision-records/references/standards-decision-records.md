...
The repo type is declared in `.ki-config.toml` under `["knowledgeislands/ki-agentic-harness:ki-decision-records"]` (or inferred from `["knowledgeislands/ki-agentic-harness:ki-kb"]` presence). The checker auto-detects the decisions directory (`docs/decisions/` then `Admin/Governance/Decisions/`) and picks the matching index file by mode; pass an explicit path to override.
