...
import { createFeatureDefinitionsSession } from './feature-definitions.ts'
...
const identityContext = (session: ReturnType<typeof createFeatureDefinitionsSession>) => {
  const subject = session.subjects[0]
  if (!subject) throw new Error('ki-feature-definitions session did not expose its repository subject')
...
test('audit is read-only and returns one stable prepared context', () => {
  ...
  const session = createFeatureDefinitionsSession(options(repository, 'audit'))
  const subject = session.subjects[0]
...
test('the ID item coalesces heading normalization into one session-owned proposal', () => {
  const { repository, area, original } = fixture()
  const session = createFeatureDefinitionsSession(options(repository, 'conform'))
  const context = identityContext(session)
...
test('a symlinked features directory is not traversed', () => {
  const repository = temporaryDirectory('ki-feature-definitions-linked-root-')
  const outside = temporaryDirectory('ki-feature-definitions-linked-target-')
  mkdirSync(join(repository, 'docs'), { recursive: true })
  writeFileSync(join(outside, 'authentication.md'), '### AUTH-001 - Outside\n')
  symlinkSync(outside, join(repository, 'docs', 'features'))
  const session = createFeatureDefinitionsSession(options(repository, 'conform'))
