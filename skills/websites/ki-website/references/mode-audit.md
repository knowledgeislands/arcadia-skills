...
2. **Run the mechanical rubric.** Run `ki repo audit --repo <repo> --skill ki-website`. It inspects the Eleventy dependency and configuration, config-less Tailwind entry, site source layout, script family, SEO partial, portable output seam, generated-output ignore, and `[skills.ki-website]` opt-in. Capture its output verbatim.
