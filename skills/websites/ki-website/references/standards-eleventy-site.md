...
- The build emits **`./dist`** — i.e. **`dist/` lives inside the `site/` workspace** (`site/dist/`). Each workspace owns its own output directory; no cross-workspace output coupling. The hosting Worker (`ki-website-cloudflare` §2) points `assets.directory` at `./dist` from `site/wrangler.jsonc`.
...
The build's output, and the **only** thing `ki-website-cloudflare` needs:
...
`dist/` sits inside the `site/` workspace (`site/dist/`), per §2. The hosting skill points `assets.directory` at `./dist` from `site/wrangler.jsonc`. **Building `dist/` is this skill; serving it is `ki-website-cloudflare`.**
