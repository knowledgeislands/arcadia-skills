...
2. Treat `assets.directory` as the seam to the `dist/` emitted by `ki-website`.
...
Selecting this capability runs its declared `ki-website` prerequisite first. `ki-engineering` is independently coverage-detected, and the unscoped repository audit includes it when declared:

```text
ki repo audit --skill ki-engineering --repo <repo>
ki repo audit --skill ki-website --repo <repo>
...
- Building `dist/`, Eleventy, Nunjucks, Tailwind, and portable URLs → `ki-website`.
