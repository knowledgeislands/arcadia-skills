...
3. Apply the reviewed local edits in place: correct the site `wrangler.jsonc`, the `assets.directory` seam, package scripts, `.gitignore`, and the `[skills.ki-website-cloudflare]` table. Never infer an account, domain, Worker name, or deploy target.
