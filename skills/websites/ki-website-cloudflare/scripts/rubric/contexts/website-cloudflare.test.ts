...
const makeRoot = (): string => {
  const root = mkdtempSync(join(tmpdir(), 'ki-website-cloudflare-'))
  roots.push(root)
  return root
}
...
const writeCanonicalRepository = (repository: string): void => {
  ...
  writeFileSync(join(repository, '.ki-config.toml'), '[skills.ki-website-cloudflare]\n')
  writeFileSync(
...
describe('ki-website-cloudflare session', () => {
