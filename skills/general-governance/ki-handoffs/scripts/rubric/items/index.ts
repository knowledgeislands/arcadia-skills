...
import { KI_HANDOFFS_RUBRIC } from './catalogue.ts'
...
const catalogueDefinition = KI_HANDOFFS_RUBRIC
  ...
  families: [
    {
      ...
      items: [
        ...
        },
        {
          ...
          conform: (context: NativeHandoffsContext) => ({
...
const runtimeItems = new Map<string, NativeRuntimeItem>(
  runtimeDefinition.families.flatMap((family) => family.items.map((item) => [item.code, item] as const))
)
...
export default {
  ...
  createContext: runtimeDefinition.createContext,
  families: catalogueDefinition.families.map((family) => ({
    ...family,
    selectContext: (context: unknown) => context,
    items: family.items.map((item) => {
      const runtime = runtimeItems.get(item.code)
      if (!runtime) throw new Error(`${item.code} has no native runtime`)
      return directItem(item, runtime)
    })
  }))
} as const
