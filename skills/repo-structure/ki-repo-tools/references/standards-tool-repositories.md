...
- Put a literal `\&` line immediately after every `.SH` and `.SS`, followed by `.PP` before prose or another structural macro. Never place bare prose or a structural macro immediately after a heading. This gives the rendered heading a clear visual separation without relying on renderer-specific blank-line behaviour.
