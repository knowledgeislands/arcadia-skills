...
The full, quotable standard is [the Eleventy site standard](references/standards.md); the line-by-line pass/fail items are in [the audit rubric](references/rubric.md); the tracked provenance is [the source list](references/sources.md). `ki repo audit --skill ki-website` runs the mechanical checks. Read those for detail; this file is the operating procedure.
...
ki repo audit --repo <repo> --skill ki-engineering          → common toolchain
  then ki repo audit --repo <repo> --skill ki-website       → site-build delta (THIS skill)
...
1. **Run the common layer first.** `ki repo audit --repo <repo> --skill ki-engineering` covers the shared toolchain. Don't re-derive it here.
2. **Run the mechanical checker.** `ki repo audit --repo <repo> --skill ki-website` locates the site root (the `site/` workspace), then reports: the `@11ty/eleventy` dep, **no `tailwind.config.*`**, `eleventy.config.ts` present, the relative-URL transform, the `.ts` + `.json5` data extensions, the Tailwind `eleventy.before` hook, `main.css` importing `tailwindcss`, the `src/` layout dirs, the build/dev script family, the `seo-meta` partial, and `dist/` gitignored. It emits the unified severity ladder and exits non-zero on any FAIL M finding. Capture its output verbatim.
...
2. Fix the gaps in place — use the canonical shape from [the standard](references/standards.md): the lean layout (§2–§5) and the fuller patterns (tokens, layouts, SEO — §5–§7) as needed. `ki repo conform --repo <repo> --skill ki-website` can add the `[ki-website]` marker and correct the generated-output ignore entry; scaffold and deployment decisions remain explicit work.
...
Use the canonical shape from **[the standard](references/standards.md)** over inventing: the `eleventy.config.ts` patterns (§4), the `main.css`/`tokens.css` pair (§5), the `_includes/{layouts,partials}/` shells (§3), and the build/dev script family (§8). Adapt names, palette, and content; keep the four invariants from day one. Declare the `[ki-website]` table, then run the native audit. For the toolchain scaffold defer to `ki-engineering` EDUCATE; for hosting, `ki-website-cloudflare` EDUCATE.
...
2. **Re-fetch each** (WebFetch / WebSearch) and diff against the standard + rubric + native rubric definition: an Eleventy 3.x API change, a Tailwind-4 `@theme`/`@import` change, a data-extension or transform-API shift.
