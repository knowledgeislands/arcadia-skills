...
**Divergence:** default access level is `read` everywhere except `mcp-ki-repo-kb-notion-mirror`, which defaults to `write` (it ships no read-only tools) — intentional, do not flag.
