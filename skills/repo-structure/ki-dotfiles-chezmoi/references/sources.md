...
The authoritative sources behind [the standard](standards.md), [the rubric](rubric.md), and its native [rubric contract](../scripts/rubric/index.ts). Mode REFRESH reads this file, re-fetches each source, diffs it against the standard and contract, then **bumps the `last reviewed` dates** and refreshes the `## Last review` block below (what changed is recorded in the commit, not a changelog).
