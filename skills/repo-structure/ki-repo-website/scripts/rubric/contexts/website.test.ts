...
const fixture = (): string => {
  const repository = temporaryDirectory('ki-repo-website-')
...
const rootContext = (session: ReturnType<typeof createWebsiteSession>) => {
  ...
  if (!subject) throw new Error('ki-repo-website session did not expose its repository subject')
...
test('symlinked proposal targets are never traversed or replaced', () => {
  const repository = fixture()
  const outside = temporaryDirectory('ki-repo-website-outside-')
...
test('a symlinked Eleventy marker activates reporting without exposing its contents', () => {
  const repository = temporaryDirectory('ki-repo-website-root-')
  const outside = temporaryDirectory('ki-repo-website-config-')
