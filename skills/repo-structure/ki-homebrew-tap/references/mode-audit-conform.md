...
2. **Run the mechanical checker.** `ki repo audit --skill ki-homebrew-tap <tap-path>` checks `Formula/` presence, per-formula class/fields/desc-style/versioned-url, the README table, and the `[ki-homebrew-tap]` marker. Native audit does not invoke Homebrew; run its commands explicitly in the next step.
3. **Also run `ki-repo`'s audit** — the tap is first a repo: `ki repo audit <tap-path>` covers README/LICENSE/`.gitignore`/GitHub settings/security. The tap is clean only when both pass. It does **not** run `ki-engineering` (no `package.json` toolchain — the `ki-plugins` pattern).
...
5. **Do the judgment pass the script can't** — walk [Audit Rubric](rubric.md)'s **[J]** items:
...
3. Add the `[ki-homebrew-tap]` marker after `[ki-repo]` has established `.ki-config.toml` — `ki repo conform --skill ki-homebrew-tap <tap-path>` adds it safely. Run `ki-repo conform <tap-path>` for the repo-level files/settings.
4. **Re-run `brew` locally if it is installed** — `brew style Formula/<tool>.rb` and `brew audit --strict <tool>` must pass — then re-run the checker until clean (0 FAIL). Do not hand off while `brew` reports issues.
