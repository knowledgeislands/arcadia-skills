...
The full, quotable standard lives in [Homebrew tap standard](references/standards.md); the line-by-line pass/fail items in [Audit Rubric](references/rubric.md). `ki repo audit` and `ki repo conform` execute the mechanical contract. Read those when you need detail; this file is the operating procedure.
...
This skill **rides `ki-repo`** (the tap is first a git repo: README, LICENSE, `.gitignore`, GitHub settings, security — all `ki-repo`'s) and declares that edge, but it does **not** ride `ki-engineering`: a tap has no `package.json`/TypeScript toolchain, so a bare `[ki-repo]` + `[ki-homebrew-tap]` config is complete (the `ki-plugins` precedent). It declares `ki-depends-on: []`.
...
- Marker `[ki-homebrew-tap]` is a **keyless opt-in table**, validate-down (like `[ki-mcp]`): its presence is the whole config; any key under it is unknown and WARNed.
- Checker output uses the canonical JSONL response by default. Pass `--reporter=terminal` for the filtered human report.
