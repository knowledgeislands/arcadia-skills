...
_On-demand procedure for tools' REFRESH mode. The cadence and source list are declared in [`sources.md`](sources.md). The full standard lives in [`standards.md`](standards.md)._
...
2. **Re-fetch each source** (WebFetch/WebSearch) and **diff against the [standard](standards.md) + [rubric](rubric.md) + [`scripts/rubric/index.ts`](../scripts/rubric/index.ts)**. Look for: changed shellcheck defaults/severities, bats-core API or install changes, a new keep-a-changelog / semver revision, XDG spec changes, and any new capability signal the reference repo has adopted.
...
5. **Propose a diff** to the standard and native rubric definition (where a check became mechanical). Every new **[M]** item must land as a native checker check. Confirm before writing, then republish [the rubric](rubric.md) with `ki skill rubric ki-tools --write`.
