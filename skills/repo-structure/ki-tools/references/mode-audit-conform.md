...
2. Fix the gaps in place — **copy from `tools-mgit`** (the reference repo) rather than invent:
   - Missing exec bit → `ki repo conform --repo <repo-path> --skill ki-tools` (and commit — git tracks it).
   ...
   - Missing `[ki-tools]` marker → `ki repo conform --repo <repo-path> --skill ki-tools` when `.ki-config.toml` already parses (then tidy).
