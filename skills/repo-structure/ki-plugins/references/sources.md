...
The authoritative sources behind the [Plugins Standard](standards.md) and [Audit Rubric](rubric.md). Mode REFRESH reads this file, re-fetches each source, diffs it against the standard + rubric + native rubric definition, then **bumps the `last reviewed` dates** and refreshes the `## Last review` block below (what changed is recorded in the commit, not a changelog). Regenerate the published rubric with `ki skill rubric ki-plugins --write`. This is the skill's memory of where the standard comes from — keep it current.
