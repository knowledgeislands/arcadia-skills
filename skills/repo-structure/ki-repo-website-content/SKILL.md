...
```text
<repo>/                             # monorepo — root package.json workspaces: ["workspaces/*"] from day one
├── package.json                    # workspaces: ["workspaces/*"] — the site is always its own workspace package
└── workspaces/
    └── site/                       # the site workspace, workspaces/site (never a flat repo-root layout)
...
A companion deployable (a bot, an ingress Worker — out of this skill's scope) is a pure **addition** to the `workspaces/` folder (`workspaces/ingress` beside `workspaces/site`), not the reason workspaces appear. Each workspace owns its own `dist/`; `workspaces/site` always emits to `./dist` (`workspaces/site/dist/`).
...
- Hosted conform is intentionally narrow: only the contained, physical `.ki.toml` contribution is eligible for this skill's proposals. `ki-repo` centrally composes the root `.gitignore`; application scaffolding, builds, deployment, and external commands remain explicit.
