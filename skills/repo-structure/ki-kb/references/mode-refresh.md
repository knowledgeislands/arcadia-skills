...
1. **Read [the source list](sources.md)** - the canonical structure definition in [the Knowledge Islands knowledge-base standard](standards-knowledge-base.md), [the frontmatter standard](standards-frontmatter.md), and the bases actively using this skill, each with a `last reviewed` date.
