...
This skill **wraps an external standard.** Homebrew already defines what a valid tap and formula are (the Formula Cookbook, `brew audit`, `brew style`); this skill does not re-invent that. It checks the tap's **shape**—the things a tap needs beyond a single valid formula—and, when Homebrew is available, runs `brew style` and `brew audit --strict` against every formula. So a finding is either **shape** (this skill's house convention for the tap: a `Formula/` directory, a README formulae table, a versioned-tarball source) or **spec** (Homebrew's own rule, surfaced by `brew`). Never present a shape preference as a Homebrew "MUST"; when unsure which a rule is, run Mode REFRESH against the Cookbook.
...
- `TAP-7` runs `brew style` and `brew audit --strict` with automatic Homebrew updates disabled. A workspace clone is valid when each formula matches the active local tap; otherwise it warns rather than applying an audit from different source. If Homebrew is unavailable, `brew test-bot` CI remains the backstop.
