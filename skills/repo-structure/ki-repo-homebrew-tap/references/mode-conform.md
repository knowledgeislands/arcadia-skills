...
4. **Resolve Homebrew validation only when necessary.** AUDIT already runs `brew style` and `brew audit --strict` when a workspace formula matches the active local tap. If it reports Homebrew unavailable or stale source, restore that prerequisite and re-run AUDIT; do not assume a check of different formula source applies here.
