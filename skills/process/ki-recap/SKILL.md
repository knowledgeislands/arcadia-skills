...
2. **Surface what is outstanding** — unfinished threads, deferred fixes, and work that should land in the repository's forward-work structure: `ROADMAP.md` / `ki-plan` for non-KB repositories, Streams / a proposal Checklist for Knowledge Bases. Always check whether the session's work is fully committed — dirty files this session touched are outstanding; dirty files from other threads are out of scope. A roadmap item or Stream **added this session** is "what happened", not outstanding.
