...
3. Run the repository's read-only `ki-roadmap` audit against the git root. Use its declared package script or vendored checker; if neither is available, stop. Continue only with zero FAIL and zero WARN. Do not run CONFORM or repair unrelated roadmap state.
