...
- [`accept <REPO>-<THEME>-<NNN>`](#accept-repo-theme-nnn)
- [`done <REPO>-<THEME>-<NNN>`](#done-repo-theme-nnn)
- [`ready <REPO>-<THEME>-<NNN>...`](#ready-repo-theme-nnn)
- [`prune [theme]`](#prune-theme)
- [`execute <REPO>-<THEME>-<NNN>...`](#execute-repo-theme-nnn)
- [`new <theme> <title>`](#new-theme-title)
- [`promote`](#runtime-binding-promote)
- [`status [theme]`](#status-theme)
- [Mandate](#mandate)

## `accept <REPO>-<THEME>-<NNN>`

1. Require the thematic profile. Parse the plan identifier as its configured uppercase repository code, uppercase theme code, and zero-padded serial, `<REPO>-<THEME>-<NNN>`. Locate the one `<REPO>-<THEME>-<NNN>-*.md` under the corresponding theme's `plans/` directory; stop on zero or multiple matches. Require `status: in-progress` and a canonical `Blocking` or `Next` item; a plan already in `acceptance` is shown as pending review, and an open plan must first be readied and executed.
...
4. Derive the lowercase hyphenated `<slug>` from the remaining title, no longer than 50 characters. If the theme's `plans/` directory is absent, create it only after revalidating the canonical theme directory, then validate the new directory immediately. Prepare `docs/roadmap/<theme>/plans/<REPO>-<THEME>-<NNN>-<slug>.md` using `ki-roadmap`'s plan format with `status: open` and `baseline-ref: —`; include the confirmed `transferred-from` only when step 2 established transferred provenance. Add the canonical final `**Plan:** [<REPO>-<THEME>-<NNN>](plans/<REPO>-<THEME>-<NNN>-<slug>.md)` line on the selected roadmap item. Fill Steps with concrete, checkable actions; fill the rest from context, marking genuine gaps `<!-- TODO -->`. When planned execution needs workers, include `## Delegation` with classified rounds, bounded worker/file scopes, and gates; otherwise omit it.
