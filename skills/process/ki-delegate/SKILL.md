...
**Kind:** process. Turns a task list or an approved plan into a **tiered, round-sequenced execution** run across sub-agents; the model-tier cost/selection policy it draws on is owned by `ki-tokenomics`. Full procedure in [the delegation standard](references/standards-delegation.md).
...
**Operating invariant — bounded, visible delegation with a protected orchestrator lane.** The orchestrator remains available for caller steering, decisions, progress, review, and integration while workers own implementation lanes; it does not take a worker-sized implementation task merely to consume capacity. Give every worker one bounded deliverable, file boundary, verification gate, and expected completion checkpoint. Update the caller before dispatch, after a meaningful dispatch, when the trajectory of a multi-agent round materially changes, when a unit passes its gate, and on every material blocker. While work remains open, also update at least every few minutes when no cadence is agreed. Await a worker's completion rather than repeatedly polling worker lists; if its checkpoint is missed, make one purposeful status check, then wait, re-scope, or stop the unit. Each update names what completed, what is currently running, the next checkpoint, and any blocker or changed estimate. A terminal result never substitutes for timely intermediate updates. Once the delegated implementation has passed its gate and reaches its acceptance review (or is recorded Done where no review applies), end the caller-facing completion update with the compact celebration banner in the procedure.
...
- Installed as a core user skill by `ki bootstrap` — usable in any repo on the machine. Like `ki-bootstrap`, never vendored or declared in a repo's `.ki-config.toml` — no `[ki-delegate]` table, ever.
