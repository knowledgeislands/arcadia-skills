...
- Installed as a core user skill by `ki bootstrap`, alongside `ki-bootstrap`, `ki-recap`, `ki-plan`, and `ki-delegate`. It is never vendored or declared in a repository's `.ki-config.toml`: no `[ki-next]` table.
