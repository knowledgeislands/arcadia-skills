...
ki-depends-on: []
description: >
  Selects, defers, and prepares roadmap work in a non-KB repository: re-grounds the roadmap, triages inbound handoffs, optionally reviews relevance, applies user-confirmed horizon promotion or deferral, and creates review-first governed plans through `ki-plan`. A process skill (kind: process, ADR-KI-HARNESS-SKILLS-006): it applies the transition rules owned by `ki-roadmap`; it does not hold a separate roadmap standard. Installable globally, cross-repo — usable in any non-KB repository on the machine. Triggers: "what should we do next", "pick the next roadmap item", "defer this roadmap item", "plan the next work", "/ki-next". Not a session recap (`ki-recap`), a roadmap checker or horizon authority (`ki-roadmap`), or an individual plan lifecycle (`ki-plan`).
argument-hint: 'next [--review] | defer <item> <soon|waiting-for|future> | help'
---

# ki-next

**Kind:** process. Selects, defers, and prepares portfolio work through confirmed horizon transitions and governed plans. The full procedure and scenarios are in [the next-work standard](references/standards-next-work.md).
...
5. **Defer** an explicitly named item only after presenting its exact later horizon, wording, and any affected plan; it never infers a move or disposes of an ordinary plan.
6. **Plan** only confirmed Blocking or Next work, through `ki-plan`, and stop for review after creating or revising each plan.

## Relationship map

```text
ki-recap (optional, current-session context)
  └─> ki-next (portfolio selection, promotion, and deferral)
...
`help` / `-h` / `?` explains this skill and stops, taking no action. With no argument or `next`, run the full procedure. `--review` asks for the optional relevance pass before selection; it does not grant permission to change roadmap content. `defer <item> <soon|waiting-for|future>` identifies an exact proposed deferral; it still requires confirmation of the wording, destination, order, and any plan handling before it writes.
...
- The transition rules belong to `ki-roadmap`; `ki-next` applies confirmed promotion and deferral consistently. `ki-plan` owns individual-plan creation, execution, and closure.
