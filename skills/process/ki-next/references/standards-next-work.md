...
2. Ask `ki-roadmap` to identify the simple or thematic profile. Run its read-only AUDIT through the repository's declared script or its vendored checker. Stop on any FAIL or WARN; name the repair route, but do not run CONFORM or repair unrelated state.
