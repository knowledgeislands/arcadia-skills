...
Reviews acceptance evidence, records approved closure, retains completed records, and performs exact confirmed pruning.
...
4. Prune only exact named `done` records after a separate explicit confirmation.
...
`prune <work-item>...` lists only the exact named `done` records, requires a separate explicit confirmation, then deletes only that confirmed set.
