...
- **LAY-2 [J] — Path-independent identity** — Grouping subdirectories are for human organisation only; identity is name, not path. (standards-subagent-definitions.md#2-layout, CC, HOUSE)
  - _Evidence scope:_ The target agent definition and the evidence named by this criterion.
  - _Review prompt:_ Grouping subdirectories are for human organisation only; identity is name, not path.
  - _Outcomes:_ conforming; gap; exclusion
  - _Conforming guidance:_ Revise the definition through its responsible author, record a named gap, or record an explicit justified exclusion.
...
- **NAME-1 [M] — Name present** — name is present. (standards-subagent-definitions.md#3-frontmatter-name, CC)
  - _Remediation:_ diagnostic — Correct the evidenced agent-definition issue through the responsible author; do not rewrite an agent role, prompt, authority, or runtime setting automatically.
