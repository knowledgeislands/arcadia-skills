...
const judgment = (code: string, title: string, description: string, prompt: string): RubricItem<PluginsContext> => ({
  ...
  judgment: {
    ...
    guidance:
      'Regenerate or revise the projection through its responsible owner, record a named gap, or record an explicit justified exclusion.'
  }
})
...
const PLUG_8 = mechanical(
  'PLUG-8',
  'Projected skills',
  '`<plugin>/skills/*` each carries a physical `SKILL.md`.',
  'FAIL',
  (context) =>
    active(context, () =>
      result(
...
const PLUG_9 = mechanical(
  'PLUG-9',
  'Flattened agents',
  '`<plugin>/agents/*.md` are physical flat files.',
  'FAIL',
  (context) =>
    active(context, () =>
      result(
...
const PLUG_10 = mechanical('PLUG-10', 'MCP deferral', 'No `.mcp.json` appears in the plugin.', 'WARN', (context) =>
  active(context, () =>
    result(
      context.mcpFiles.length === 0,
      'MCP payload remains deferred',
      `unexpected MCP payloads: ${context.mcpFiles.join(', ')}`
    )
  )
)
