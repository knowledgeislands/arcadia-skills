...
> **Generated publication.** The TypeScript rubric items under `scripts/rubric/items/` are canonical. Edit those definitions, then rerun `ki dev skill rubric ki-harness --write`.
...
- **LONG-1 [J] — Refresh path** — The ki-harness skill carries REFRESH and a dated source review record. (standards-compatible-harness.md)
  - _Evidence scope:_ The ki-harness REFRESH procedure, source list, cadence, and current compatible-harness standard.
