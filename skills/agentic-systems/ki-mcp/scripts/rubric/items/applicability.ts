...
const KI_CONFIG: RubricItem<McpApplicabilityContext> = {
  ...
  description:
    'A repository is applicable when it declares [skills.ki-mcp] or contains src/mcp-server/. Otherwise the audit emits one NOT_APPLICABLE finding and stops; declared keys are rejected because this skill has no configuration options.',
  sources: [STANDARD],
  mechanical: {
    level: 'WARN',
    overrideLevels: ['FAIL'],
    ...
    audit: {
      ...
      run: (context) => {
        ...
        if (!context.applicable)
          return outcome(
            'NOT_APPLICABLE',
            'ki-mcp not applicable: no [skills.ki-mcp] declaration or src/mcp-server/ structural marker.',
            context.root
          )
        ...
        if (context.config === 'malformed')
          return outcome(
            'VIOLATION',
            '.ki-config.toml is malformed; repair it before adding [skills.ki-mcp].',
            '.ki-config.toml'
          )
        if (context.config === 'absent')
          return outcome(
            'VIOLATION',
            'No [skills.ki-mcp] table; add it to mark this repository as governed.',
            '.ki-config.toml'
          )
        return context.configKeys.length > 0
          ? outcome(
              'VIOLATION',
              `Unknown keys under [skills.ki-mcp]: ${context.configKeys.join(', ')} (validate-down).`,
              '.ki-config.toml'
            )
          : outcome('PASS', '[skills.ki-mcp] table is present.', '.ki-config.toml')
      }
    },
    conform: {
      phase: 'PRIMARY',
